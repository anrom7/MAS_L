...
public class FIPARequestInitiatorAgent extends Agent {
  ...
  protected String sendMessage(String str) {
	 
	  		// Процедура відправлення ACL повідомлення
			msg.setProtocol(FIPANames.InteractionProtocol.FIPA_REQUEST);
			// We want to receive a reply in 10 secs
			msg.setReplyByDate(new Date(System.currentTimeMillis() + 10000));
			//Задання контенту повідомлення
			msg.setContent(str);
			...
			addBehaviour(new AchieveREInitiator(this, msg) {
				protected void handleInform(ACLMessage inform) {
					...
					igui.textField_2.setText(inform.getContent());
				}
				protected void handleRefuse(ACLMessage refuse) {
					System.out.println("Agent "+refuse.getSender().getName()+" refused to perform the requested action");
					nResponders--;
				
					result+=";"+refuse.getContent();
					System.out.println("resu: "+result);
