...
public class BrokerAgent extends Agent {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2794140625233846516L;
	private AID responder;
	private String [] stringsMas = new String [100];
	private Integer NRes= 0 ;
  	...
  	if (args != null && args.length > 0) {
  		...
  		for (int i = 0; i < args.length; i++) {
  			//responder = new AID((String) args[i], AID.ISLOCALNAME);
  		  	stringsMas[i] = (String) args[i];
  		  System.out.println(args[i]);//"" + responder;
  		  	NRes++;
			...
			arer.registerPrepareResultNotification(new AchieveREInitiator(this, null) {
				...
				protected Vector prepareRequests(ACLMessage request) {
					...
					for (int i = 0; i < NRes; i++) {
						outgoingRequest.addReceiver(new AID(stringsMas[i], AID.ISLOCALNAME));
					}
				...
				protected void handleInform(ACLMessage inform) {
					...
					storeNotification(ACLMessage.INFORM, inform.getContent());
				}
				...
				private void storeNotification(int performative, String cont) {
					Boolean boo = false;
