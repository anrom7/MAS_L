		...
		if (args != null && args.length > 0) {
			...
			arer.registerPrepareResultNotification(new AchieveREInitiator(this,
				...
				protected Vector prepareRequests(ACLMessage request) {
					...
					ACLMessage incomingRequest = (ACLMessage) getDataStore()
							.get(incomingRequestKey);
					// Prepare the request to forward to the responder
					System.out.println("Agent " + getLocalName()
							+ ": Forward the request to " + responder.getName());
					ACLMessage outgoingRequest = new ACLMessage(
							ACLMessage.REQUEST);
					outgoingRequest
					...
					outgoingRequest.setReplyByDate(incomingRequest
							.getReplyByDate());
