...
public class FIPARequestResponderAgent extends Agent {
		...
		addBehaviour(new AchieveREResponder(this, template) {
			...
			protected ACLMessage handleRequest(ACLMessage request) throws NotUnderstoodException, RefuseException {
				...
				if (checkAction(request.getContent())) {
					...
					// protocol the AGREE message is optional. Return REFUSE if you
					...
					// змінено замість trow exeption надсилається ACLMessage.REFUSE  
			...
			protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException {
				if (performAction()) {
					...
					// змінено замість trow exeption надсилається ACLMessage.FAILURE  
					System.out.println("Agent "+getLocalName()+": Action failture");	
					 ACLMessage failture = request.createReply();
					 failture.setPerformative(ACLMessage.FAILURE);
					 failture.setContent("not-available");
					 return failture;
				}	
			}
		} );
  ...
  private boolean checkAction(String s) {
