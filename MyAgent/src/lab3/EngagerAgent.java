...
import employment.BadCountry;
import employment.Company;
import employment.EmploymentOntology;
import employment.Engage;
import employment.EngagementError;
import employment.Person;
import employment.PersonTooOld;
import employment.PersonTooYoung;
	...
	*/
	...
	public ACLMessage prepareResponse(ACLMessage request) {
			...
			try{
				...
				if (addr.getCountry().equalsIgnoreCase("Russia")){
					ContentElementList l = new ContentElementList();
					l.add(a);
					l.add(new BadCountry());
					getContentManager().fillContent(temp, l);
					temp.setPerformative(ACLMessage.REFUSE);
				} else
				if (p.getAge().intValue() <= 21){
					ContentElementList l = new ContentElementList();
					l.add(a);
					l.add(new PersonTooYoung());
					getContentManager().fillContent(temp, l);
					temp.setPerformative(ACLMessage.REFUSE);
				} else if (p.getAge().intValue() < 35){
