...
import com.multiagent.employment.Address;
import com.multiagent.employment.Company;
import com.multiagent.employment.EmploymentOntology;
import com.multiagent.employment.Engage;

import com.multiagent.employment.Person;

import com.multiagent.employment.WorksFor;
...
public class RequesterAgent extends Agent {
	...
	class HandleEngagementBehaviour extends SequentialBehaviour {
		...
		public void onStart() {
			...
			try {
				...
				// Create an object representing the fact that person p works
				// for company c
				...
				// Write the works for predicate in the :content slot of the
				// message
				...
				// protocol
				queryBehaviour = new CheckAlreadyWorkingBehaviour(myAgent,
						queryMsg);
			...
			try {
				...
				if (cs.getTypeName().equals(EmploymentOntology.WORKS_FOR)) {
					...
					System.out.println("Person " + p.getName()
							+ " is already working for " + c.getName());
					...
					WorksFor wf = (WorksFor) o.toObject(cs
							.getAbsObject(SLVocabulary.NOT_WHAT));
					Person p = (Person) wf.getPerson();
					Company c = (Company) wf.getCompany();
					Engage e = new Engage();
					e.setPerson(p);
					...
					// Create and add a behaviour to request the engager agent
					// to engage
					// person p in company c following a FIPARequest protocol
					((HandleEngagementBehaviour) parent).requestBehaviour = new RequestEngagementBehaviour(
							myAgent, requestMsg);
					...
					// Inform the user
					System.out
							.println("Unexpected response from engager agent");
				}
	...
	class RequestEngagementBehaviour extends SimpleAchieveREInitiator {
		...
		protected void handleAgree(ACLMessage msg) {
			System.out
					.println("Engagement agreed. Waiting for completion notification...");
		}
		...
		protected void handleNotUnderstood(ACLMessage msg) {
			System.out
					.println("Engagement request not understood by engager agent");
		}
