...
public class EngagerAgent extends Agent {
	//MyFactory fact = null;
	//OWLOntology onto = OWLManager.createOWLOntologyManager().createOntology().getOntologyID() ;
	//fact = new MyFactory(onto);
	// AGENT BEHAVIOURS
	/**
	...
	class HandleEnganementQueriesBehaviour extends SimpleAchieveREResponder {
		...
		public HandleEnganementQueriesBehaviour(Agent myAgent) {
			super(myAgent, MessageTemplate.and(MessageTemplate
					.MatchProtocol(FIPANames.InteractionProtocol.FIPA_QUERY),
					MessageTemplate.MatchOntology(JobOfferOntologyOntology.ONTOLOGY_NAME)));
		...
		public ACLMessage prepareResponse(ACLMessage msg) {
			...
			try {
				...
				Predicate pred = (Predicate) myAgent.getContentManager()
						.extractContent(msg);
				if (!(pred instanceof Job_Vacancy)) {
				...
				JV_List wf = (JV_List) pred;
				wf.addHas_elems(elem)
				Job_Vacancy p = wf.getPerson();
				...
				if (((EngagerAgent) myAgent).isWorking(p, c))
					...
					Ontology o = getContentManager().lookupOntology(
							JobOfferOntologyOntology.ONTOLOGY_NAME);
				...
				System.err.println(myAgent.getLocalName()
						+ " getRoleName() unsucceeded. Reason:"
						+ oe.getMessage());
			}
	...
	class HandleEngageBehaviour extends SimpleAchieveREResponder {
		...
		public ACLMessage prepareResultNotification(ACLMessage request,
			...
			try {
				...
				Action a = (Action) myAgent.getContentManager().extractContent(
						request);
	...
	protected void setup() {
		System.out.println("This is the EngagerAgent representing the company "
		...
		MessageTemplate mt = MessageTemplate.and(MessageTemplate
				.MatchProtocol(FIPANames.InteractionProtocol.FIPA_REQUEST),
				MessageTemplate.MatchOntology(EmploymentOntology.NAME));
		HandleEnganementQueriesBehaviour b = new HandleEnganementQueriesBehaviour(
				this);
