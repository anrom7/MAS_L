...
import ua.edu.lp.cad.mas.lab3.occupation.OccupationOntologyOntology;
import ua.edu.lp.cad.mas.lab3.occupation.impl.DefaultOccupation;
...
public class RequesterAgent extends Agent {
	...
	class CheckAlreadyWorkingBehaviour extends SimpleAchieveREInitiator {
		...
		protected void handleInform(ACLMessage msg) {
			try{
				...
				Ontology o = myAgent.getContentManager().lookupOntology(OccupationOntologyOntology.OCCUPATION_NAME);
				if (cs.getTypeName().equals(OccupationOntologyOntology.WORKS_FOR)) {//це предикат - мені то не подобається але хз хай буде...
					...
					requestMsg.setOntology(OccupationOntologyOntology.ONTOLOGY_NAME);
	...
	*/
	...
	protected void setup() {
		...
		getContentManager().registerOntology(OccupationOntologyOntology.getInstance());
